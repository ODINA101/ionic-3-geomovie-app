...
/**
 * Generated class for the KomediurPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-komediur',
  templateUrl: 'komediur.html',
})
export class KomediurPage {
  movies = [];
  constructor(public loadingCtrl:LoadingController,public navCtrl: NavController, public navParams: NavParams,public http:Http,public alertCtrl:AlertController) {
    ...
    this.http.get('https://myionicapp.000webhostapp.com/api.php?q=komediur')
    .map(res => res.json())
    .subscribe(res => {
  this.movies = res;
  loader.dismiss();
